...
def get_game(game, username):
    ...
    return {'game': game, 'runner_name': runner.title, 'corp_name': corp.title, 'endgame_text': endgame_string, 'player_side': player_side}

@login_required(login_url=settings.LOGIN_URL)
def view_game(request, game_id):
    game = get_object_or_404(Game, pk=game_id)
    return render(request, 'games/game_details.html', get_game(game, request.user.username))
